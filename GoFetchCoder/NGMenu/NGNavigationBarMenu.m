...
@implementation NGNavbarMenuItem
...
@implementation NGNavigationBarMenu
...
- (instancetype)initWithItems:(NSArray *)items width:(CGFloat)width maximumNumberInRow:(NSInteger)max {
    ...
    self.NG_height = (_numberOfRow+1) * rowHeight;
    self.NG_y = -self.NG_height;
...
- (void)layoutSubviews {
    ...
    CGFloat buttonWidth = self.NG_width/self.maximumNumberInRow;
    CGFloat buttonHeight = rowHeight;
    [self.items enumerateObjectsUsingBlock:^(NGNavbarMenuItem *obj, NSUInteger idx, BOOL *stop) {
...
- (void)showInNavigationController:(UINavigationController *)nvc {
    ...
    if (CGRectEqualToRect(self.beforeAnimationFrame, self.afterAnimationFrame)) {
        ...
        tmp.origin.y += ([UIApplication sharedApplication].statusBarFrame.size.height+nvc.navigationBar.NG_height+rowHeight*self.numberOfRow);
    ...
    [UIView animateWithDuration:0.5
         ...
         usingSpringWithDamping:0.6
          initialSpringVelocity:1.0
                     ...
                     animations:^{
                         self.NG_y = self.afterAnimationFrame.origin.y;
    ...
    if (animation) {
        [UIView animateWithDuration:0.2 animations:^{
            ...
            [UIView animateWithDuration:0.1 animations:^{
                self.NG_y = self.beforeAnimationFrame.origin.y;
            } completion:^(BOOL finished) {
                completion();
            }];
        }];
